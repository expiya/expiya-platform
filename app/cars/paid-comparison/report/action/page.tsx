...
function PaidReportActionContent() {
  ...
  return <main className="mx-auto max-w-xl px-5 py-20"><h1 className="text-2xl font-semibold">Güvenli talep bağlantısı hazırlanıyor</h1><p className="mt-3 text-neutral-600">Rapor erişiminiz, seçtiğiniz araç ve önceki karar sohbetiniz doğrulanıyor. Henüz bayiye veri gönderilmez.</p>{visibleError&&<p className="mt-5 rounded-xl bg-red-50 p-4 text-red-800">{visibleError} Raporu aynı tarayıcıda açıp yeniden deneyin.</p>}</main>;
}

export default function PaidReportActionPage() {
  return (
    <Suspense fallback={<main className="mx-auto max-w-xl px-5 py-20"><h1 className="text-2xl font-semibold">Güvenli talep bağlantısı hazırlanıyor</h1></main>}>
      <PaidReportActionContent />
    </Suspense>
  );
}

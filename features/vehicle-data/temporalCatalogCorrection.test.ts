...
const sha256File = (path: string) => `sha256:${createHash("sha256").update(readFileSync(path)).digest("hex")}`;
...
describe("v0.55.1 temporal correction stack", () => {
  ...
  it("changes only the catalog release temporal envelope", () => {
    ...
    expect(new Date(newManifest.effective_as_of).getTime()).toBeLessThanOrEqual(new Date(activeCatalog.activated_at).getTime());
    expect(new Date(activeCatalog.activated_at).getTime()).toBeLessThanOrEqual(new Date("2026-08-16T19:33:14.000Z").getTime());
  });
  ...
  it("keeps hashes, lineage, rollback and decision facets coherent", () => {
    ...
    expect(activeCatalog).toMatchObject({
      ...
      catalog_payload_hash: newManifest.catalog_payload_hash,
      previous_active_release: "0.55.0",
      rollback_release: "0.3.0",
    });
  ...
  it("preserves the daily-life content in a compatibility-only rebind", () => {
    ...
    expect(activeDailyLife).toMatchObject({
      activeTechnicalDailyLifeRelease: newDailyLife.metadata.dailyLifeLayerVersion,
  ...
  it("preserves approved safe persona projections in a compatibility-only rebind", () => {
    ...
    expect(activePersona).toMatchObject({
      activeReleaseVersion: newPersona.releaseVersion,
      compatibleCatalogRelease: "v0.55.1",
      compatibleCatalogFingerprint: newManifest.catalog_payload_hash,
      payloadSha256: sha256File("data/production/personas/safe-traits/releases/v1.0.2-catalog-v0.55.1-2026-08-16/vehicle-persona-safe-traits.json"),
    });

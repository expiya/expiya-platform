...
export function createTemporalCorrectionReleasePayload(records:readonly PublishedVehicleRecord[]):ProductionCatalogReleasePayload{return laterPayload(records,577,TEMPORAL_CORRECTION_CATALOG_RELEASE_AS_OF)}
export function createTemporalCorrectionReleaseManifest(payload:ProductionCatalogReleasePayload):ProductionCatalogReleaseManifest{return{catalog_release_version:TEMPORAL_CORRECTION_CATALOG_RELEASE_VERSION,catalog_schema_version:CATALOG_SCHEMA_VERSION,catalog_payload_hash:catalogPayloadHash(serializeCanonical(payload)),market:"TR",source_revision:TEMPORAL_CORRECTION_CATALOG_SOURCE_REVISION,source_path:TEMPORAL_CORRECTION_CATALOG_SOURCE_PATH,effective_as_of:TEMPORAL_CORRECTION_CATALOG_RELEASE_AS_OF,record_count:payload.records.length,publishable_record_count:payload.records.length,included_variant_ids:payload.records.map(({variant})=>variant.id).sort(),generator_version:CATALOG_GENERATOR_VERSION,validator_version:CATALOG_VALIDATOR_VERSION,validator_status:"PASS",approval:{state:"APPROVED",at:TEMPORAL_CORRECTION_CATALOG_RELEASE_AS_OF,reference:"user-directed-commercial-brand-batch-01"},staging:{state:"STAGED",at:TEMPORAL_CORRECTION_CATALOG_RELEASE_AS_OF,actor_reference:"git-84b5fe8f-v0.55.0-production-activation",target:"INTERNAL_INTEGRATION_NON_PRODUCTION"},previous_release:FIFTY_FIFTH_CATALOG_RELEASE_VERSION,declared_limitations:["temporal-only-patch-of-v0.55.0","control-timestamp-evidenced-by-git-84b5fe8f-2026-08-16T09:40:33Z","record-level-provenance-and-price-observation-timestamps-preserved-for-semantic-content-equality","estimated-prices-remain-internal-only","rollback-target-is-last-loader-effective-release-v0.3.0-at-2026-08-16T12:00:00Z"]}}
...
export function validateProductionCatalogRelease(
  ...
  const newestSourcePaths:Record<string,string>={
    ...
    [FIFTY_FIFTH_CATALOG_RELEASE_VERSION]:FIFTY_FIFTH_CATALOG_SOURCE_PATH,[TEMPORAL_CORRECTION_CATALOG_RELEASE_VERSION]:TEMPORAL_CORRECTION_CATALOG_SOURCE_PATH,
...
export function validateProductionCatalogActivation(
  ...
  if (!/^\d+\.\d+\.\d+$/u.test(activation.rollback_release) || activation.rollback_release === activation.active_catalog_release_version) errors.push("ROLLBACK_RELEASE_INVALID");
  if (manifest.catalog_release_version === TEMPORAL_CORRECTION_CATALOG_RELEASE_VERSION && activation.rollback_release !== TEMPORAL_CORRECTION_ROLLBACK_VERSION) errors.push("TEMPORAL_CORRECTION_ROLLBACK_INVALID");

...
import activeManifest from "@/data/production/catalog/releases/v0.55.4/manifest.json";
...
describe("resolveRecommendationCatalog", () => {
  ...
  it("requires an explicit production setting", () => {
    ...
    const catalog = resolveRecommendationCatalog("production", new Date("2026-08-20T00:00:00.000Z"));
    expect(catalog.cars).toHaveLength(activeManifest.publishable_record_count);
    expect(catalog.cars).toHaveLength(549);
    ...
    expect(quarantineRegistry.records).toHaveLength(17);

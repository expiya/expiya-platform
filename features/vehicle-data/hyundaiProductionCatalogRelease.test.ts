...
describe("Hyundai catalog expansion release v0.3.0", () => {
  ...
  it("remains present after the active catalog advances beyond the immutable Hyundai release", () => {
    ...
    const cars = resolveRecommendationCatalog("production", new Date("2026-09-01T01:00:00.000Z")).cars;
    expect(cars).toHaveLength(activeCatalogManifest.publishable_record_count);
    expect(cars.some(({ brand }) => brand === "Hyundai")).toBe(true);
  });
});

...
describe("single Equipment pilot launch preparation", () => {
  ...
  it("checks every runtime contract fingerprint without embedding source bytes", () => {
    expect(appendix.sourceEmbedding).toBe(false); expect(appendix.contracts.length).toBeGreaterThanOrEqual(18);
    for (const contract of appendix.contracts) expect(sha(readFileSync(join(root, contract.sourcePath), "utf8"))).toBe(contract.sourceChecksum);

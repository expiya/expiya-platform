...
import dryRun from "@/data/production/equipment-public-explanation-integration/governance/launch-preparations/EPEI-PILOT-LAUNCH-V9/launch-dry-run.json";
import appendix from "@/data/production/rec-offer-audit-foundation/releases/v1.0.7-catalog-v0.55.4-2026-08-22/runtime-contract-appendix.json";
import foundationManifest from "@/data/production/rec-offer-audit-foundation/releases/v1.0.7-catalog-v0.55.4-2026-08-22/manifest.json";

const root = process.cwd(); const sha = (text: string) => `sha256:${createHash("sha256").update(text).digest("hex")}`;
const launchDir = join(root, "data/production/equipment-public-explanation-integration/governance/launch-preparations/EPEI-PILOT-LAUNCH-V9");
...
describe("single Equipment pilot launch preparation", () => {
  ...
  it("checksum-binds the immutable runtime contract snapshot without embedding source bytes", () => {
    ...
    expect(sha(readFileSync(join(root, "data/production/rec-offer-audit-foundation/releases/v1.0.7-catalog-v0.55.4-2026-08-22/runtime-contract-appendix.json"), "utf8"))).toBe(foundationManifest.runtimeContractAppendixChecksum);

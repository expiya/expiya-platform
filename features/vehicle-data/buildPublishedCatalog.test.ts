...
describe("buildPublishedCatalog", () => {
  it("publishes only complete records with active prices", () => {
    ...
    expect(catalog.records.map(({ variant }) => variant.model.value)).toEqual(["TUCSON", "IONIQ 5", "IONIQ 9", "Yaris", "Yaris"]);
  ...
  it("expires campaigns while retaining an open-ended list-price observation", () => {
    ...
    expect(catalog.records.map(({ variant }) => variant.model.value)).toEqual(["Yaris", "Yaris"]);

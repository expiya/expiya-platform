...
export function detectHumanContext(userText: string): HumanContextPolicyMatch | null {
  ...
  if (/(?:artık bekar değilim|evlendim|çocuk geliyor|bebeğimiz olacak|ailemiz büyüyor|hayatım değişti)/u.test(text)) return Object.freeze({ kind: "LIFE_CHANGE", safeAcknowledgement: "Hayatındaki değişimi hesaba katalım; yeni günlük ihtiyaçlarını araç seçiminde öne çıkarabiliriz." });

...
import { enrichAuthoritativePlanWithAutomotiveSemantics, interpretDeterministicAutomotiveSemanticAnswer, interpretDeterministicCatalogComparison, interpretDeterministicCatalogOverview, interpretDeterministicCatalogSuperlative, interpretDeterministicControlledVehicleRequest, interpretDeterministicMaterialQuestionAnswer, interpretDeterministicModelLookup, interpretDeterministicModelSuitability, interpretDeterministicSemanticQuestionExplanation, interpretUserMessage, interpretUserMessageWithAutomotiveSemantics, isDeterministicCrossFieldQuestionAnswer, isDeterministicMaterialQuestionAnswer } from "../interpretation/service";
...
import type { AutomotiveSemanticModel } from "../semantic-intelligence";
import { createAutomotiveSemanticClarificationQuestion } from "../action/semanticIntelligenceQuestion";
import { salesConversationGuidance, salesConversationPhase } from "../action/salesConversation";
...
const semanticExplanationRequested = (userMessage: string) => /(?:bunlar|bunların|bu seçenekler|bu terimler|a segment|b segment|city car|kompakt).*(?:ne demek|anlamı|anlamıyorum|bilmiyorum)|(?:ne demek|anlamadım|bilmiyorum|açıkla|basit anlat).*(?:bunlar|bunların|seçenekler|terimler)|neden\s+ingilizce|ingilizce\s+konuş|ne\s+demek\s+istiyorsun/iu.test(userMessage);
function semanticExplanationText(stableSemanticKey: string | undefined, userMessage: string): string {
  const englishComplaint = userMessage.toLocaleLowerCase("tr-TR").includes("ingilizce");
  if (englishComplaint && /air|climate|conditioning|klima/iu.test(stableSemanticKey ?? "")) return "Haklısın; İngilizce seçenek göstermem hataydı. “Klima olsun” tercihini anladım. Temel klima, otomatik klima veya çok bölgeli klima ayrımını katalog verisi güvenilir biçimde desteklemiyorsa seni bu teknik ayrımla yormayacağım. Sıcak havada rahatlık beklentini koruyup kullanımına göre ilerleyelim.";
  if (englishComplaint) return "Haklısın; İngilizce seçenek göstermem hataydı. Teknik terimleri bilmek zorunda değilsin. Söylediğin ihtiyeti gündelik Türkçeyle ele alıp yalnız kararını gerçekten değiştirecek bir soru soracağım.";
  return "Tabii; teknik sınıf adlarını bilmen gerekmiyor. A segmenti, çoğunlukla şehir içinde kullanılan en küçük otomobilleri anlatır. B segmenti, Clio veya Polo boyutlarındaki küçük otomobillerdir ve dört kişi için daha kullanışlı olabilir. City car, şehirde kolay park edilen küçük araçların genel adıdır. Kompakt otomobil ise biraz daha geniş, günlük ve aile kullanımına daha uygun araçları ifade eder. Mikro araçlar çoğunlukla iki kişilik ve çok küçüktür. Senin için kolay park edilmesi mi, yoksa dört kişinin rahat kullanabilmesi mi daha önemli?";
}
...
export function createProductionV2TurnStages(input: { readonly repositoryRoot?: string; readonly interpreter: StructuredInterpretationModel; readonly semanticInterpreter?: AutomotiveSemanticModel; readonly realizer: NaturalRealizationModel; readonly signer?: OfferSigner; readonly offerStore?: GovernedOfferStore; readonly shadow?: boolean; readonly smokeObserver?: (value: Readonly<Record<string, unknown>>) => void }): V2TurnStages {
  ...
  return {
    ...
    interpret: (turn, previous, catalog) => {
      ...
      const openMaterialQuestion = [...(previous?.materialQuestionHistory ?? [])].reverse().find((item) => item.answerStatus === "OPEN");
      const openMaterialQuestionField = openMaterialQuestion?.field;
      if (openMaterialQuestionField === "semanticMeaning" && semanticExplanationRequested(turn.userMessage)) {
        return Promise.resolve(interpretDeterministicSemanticQuestionExplanation({ messageId: turn.messageId, userText: turn.userMessage, fieldRegistry: V2_DECISION_FIELD_REGISTRY_V1, activeFieldIds }));
      }
      if (openMaterialQuestionField === "semanticMeaning" && turn.typedOptionId?.startsWith("v2q.asil.")) {
        const semanticAnswer = interpretDeterministicAutomotiveSemanticAnswer({ messageId: turn.messageId, userText: turn.userMessage, fieldRegistry: V2_DECISION_FIELD_REGISTRY_V1, activeFieldIds });
        if (semanticAnswer) return Promise.resolve(semanticAnswer);
      }
      ...
      if (controlledVehicleRequest) return input.semanticInterpreter ? enrichAuthoritativePlanWithAutomotiveSemantics({ plan: controlledVehicleRequest, semanticModel: input.semanticInterpreter, catalog, userText: turn.userMessage, fieldRegistry: V2_DECISION_FIELD_REGISTRY_V1, activeFieldIds, openMaterialQuestionField, revealedCandidateReferences: previous?.revealedCandidateIds }) : Promise.resolve(controlledVehicleRequest);
      const interpretationInput = { model: input.interpreter, messageId: turn.messageId, userText: turn.userMessage, fieldRegistry: V2_DECISION_FIELD_REGISTRY_V1, activeFieldIds, openMaterialQuestionField, revealedCandidateReferences: previous?.revealedCandidateIds };
      return input.semanticInterpreter ? interpretUserMessageWithAutomotiveSemantics({ ...interpretationInput, semanticModel: input.semanticInterpreter, catalog }) : interpretUserMessage(interpretationInput);
    },
    ...
    evaluate: async ({ turn, memory, catalog, interpretation, now }) => {
      ...
      const explicitMultipleRequested = comparisonScope || /(?:seçenek(?:ler)?|alternatif(?:ler)?|birkaç|iki|üç|2|3)\s*(?:araç|araba|otomobil|model)|(?:araç|araba|otomobil|model)\s*(?:seçenekleri|alternatifleri)|karşılaştır/iu.test(turn.userMessage);
      const decisionOfferLimit: 1 | 3 = explicitMultipleRequested ? 3 : 1;
      ...
      const budgetShortlist = budgetShortlistApplicable ? selectBudgetNearestShortlist({ ranking, affordability: rankableAffordability, snapshot: catalog, targetAmountTry: budgetTargetAmount, maximumCandidates: decisionOfferLimit }) : undefined;
      const shortlist = budgetShortlistApplicable ? budgetShortlist! : selectRankedCandidateShortlist({ ranking, maximumCandidates: decisionOfferLimit, familyDiversity: !comparisonScope, singleRequested: exactModelPreferenceScope });
      ...
      const semanticIntelligenceQuestion = createAutomotiveSemanticClarificationQuestion({ semantics: interpretation.automotiveSemantics, memory, snapshot: catalog, candidateIds: ranking.rankedCandidateIds });
      const openSemanticQuestion = [...memory.materialQuestionHistory].reverse().find((item) => item.field === "semanticMeaning" && item.answerStatus === "OPEN");
      const semanticQuestionExplanationRequested = Boolean(openSemanticQuestion) && semanticExplanationRequested(turn.userMessage);
      const questions = technicalConflictQuestion
        ...
        : semanticIntelligenceQuestion
        ? Object.freeze({ ...generatedQuestions, unansweredDecisionFields: Object.freeze([...new Set([...generatedQuestions.unansweredDecisionFields, semanticIntelligenceQuestion.question.field])]), questionCandidates: Object.freeze([semanticIntelligenceQuestion]) })
      ...
      let action = decideConversationAction({ memory, directAnswerObligation: obligation, candidateAvailability: availability, recommendationReadiness, ranking, rankedShortlist: shortlist, conflictAnalysis: null, unansweredDecisionFields: questions.unansweredDecisionFields, questionCandidates: questions.questionCandidates, normalizedUserAct: act, policy: ACTION_POLICY_V1, catalogSnapshotAvailable: true, explicitMultipleRequested, explicitTrimComparisonRequested: comparisonScope, systemCorrectionRequired: abusiveCorrection, approximateBudgetCaveatFactIds: affordability.budgetIncreaseGuidance.some((item) => item.authority === "APPROXIMATE_INTERNAL_ESTIMATE") ? ["approximate-budget"] : [] });
      ...
      if (semanticIntelligenceQuestion && act !== "ABUSE") action = { ...action, nextState: "UNDERSTANDING_NEEDS", nextAction: { type: "ASK_MATERIAL_QUESTION" }, materialQuestion: semanticIntelligenceQuestion.question, shortlistIntent: null, policyTrace: { ...action.policyTrace, matchedRule: "ASIL_USER_CONFIRMATION_REQUIRED" } };
      const urbanCampingBridgeRequested = /kamp/iu.test(turn.userMessage)
        && /(?:[İi]şe gid(?:ip gel|iş geliş)|şehir|günlük)/u.test(turn.userMessage)
        && /(?:bozuk yol|köy yolu|asfalt dışı|kırsal)/iu.test(turn.userMessage);
      const contextualBodyQuestion = questions.questionCandidates.find((candidate) => candidate.question.field === "bodyStyle");
      const contextualSuvCandidateIds = ranking.rankedCandidateIds.filter((candidateId) => {
        const bodyStyle = catalog.variantById.get(candidateId)?.decisionFacts.bodyStyle.value;
        return bodyStyle === "SUV" || bodyStyle === "Crossover";
      });
      const suvOption = contextualBodyQuestion?.question.options.find((option) => option.semanticValue === "SUV") ?? (contextualSuvCandidateIds.length > 0 ? Object.freeze({
        id: "v2q.contextual.urban-camping-suv.yes",
        semanticValue: "SUV",
        userFacingLabel: "SUV/crossover araçları değerlendirelim",
        userFacingDescription: "Şehir içi kullanım ile bozuk kamp yolları arasında pratik bir denge.",
        provenance: Object.freeze({ source: "CURRENT_CANDIDATE_POOL" as const, candidatePoolFingerprint: memory.decisionFingerprint, supportingCandidateIds: Object.freeze(contextualSuvCandidateIds), authorityReference: catalog.authority.catalogFingerprint }),
      }) : undefined);
      if (urbanCampingBridgeRequested && suvOption && !semanticIntelligenceQuestion && act !== "ABUSE") {
        action = {
          ...action,
          nextState: "UNDERSTANDING_NEEDS",
          nextAction: { type: "ASK_MATERIAL_QUESTION" },
          materialQuestion: Object.freeze({
            id: `v2q.contextual.urban-camping-suv.${memory.turn + 1}`,
            stableSemanticKey: "contextualBridge.urbanCampingSUV",
            field: "bodyStyle",
            promptIntent: "CLARIFY_REQUIREMENT",
            options: Object.freeze([Object.freeze({ ...suvOption, userFacingLabel: "SUV/crossover araçları değerlendirelim" })]),
            selectionMode: "SINGLE",
            minimumSelections: 1,
            maximumSelections: 1,
            answerCapabilities: Object.freeze(["ANSWER", "SKIP", "UNKNOWN", "NOT_IMPORTANT"] as const),
            materialityReason: "Kullanıcının şehir, aile ve bozuk yol ihtiyaçlarını teknik jargon olmadan güçlü bir gövde tercihine dönüştürür.",
          }),
          shortlistIntent: null,
          policyTrace: { ...action.policyTrace, matchedRule: "WEAK_USAGE_SIGNAL_TO_CONTEXTUAL_BODY_CONFIRMATION" },
        };
      }
      if (semanticQuestionExplanationRequested) action = { ...action, nextState: "UNDERSTANDING_NEEDS", nextAction: { type: "ANSWER_DIRECTLY" }, directAnswerObligation: { kind: "TECHNICAL_EXPLANATION", sourceMessageId: turn.messageId, authorizedExplanationFactIds: ["direct-answer"], authorizedCandidateIds: [], placement: "BEFORE_MATERIAL_QUESTION" }, materialQuestion: null, shortlistIntent: null, explanationFactIds: [...new Set([...action.explanationFactIds, "direct-answer"])], policyTrace: { ...action.policyTrace, matchedRule: "EXPLAIN_OPEN_SEMANTIC_OPTIONS_IN_PLAIN_LANGUAGE", directAnswerPlacement: "BEFORE_MATERIAL_QUESTION" } };
      const governedEconomicMeaningQuestion = semanticRecoveryQuestion?.question.stableSemanticKey === "semanticRecovery.economicMeaning";
      const casualPersonalUpdate = !/[?？]/u.test(turn.userMessage)
        && !/(?:araç|araba|otomobil|suv|sedan|hatchback|yakıt|motor|şanzıman|çekiş|bütçe|fiyat|öner|istiyorum|arıyorum|bakıyorum)/iu.test(turn.userMessage)
        && /(?:(?:yarın|bugün|bu hafta).{0,50}(?:sınav|okul|iş|toplantı|randevu|doğum günü|mezuniyet|seyahat)|(?:sınavım|toplantım|randevum|doğum günüm).{0,30}var)/iu.test(turn.userMessage);
      const casualPersonalAcknowledgement = casualPersonalUpdate
        ? /sınav|okul/iu.test(turn.userMessage)
          ? "Şimdiden başarılar 🙂 Umarım sınavın güzel geçer."
          : "Anladım; umarım her şey gönlünce gider. 🙂"
        : undefined;
      const currentMessageHasDecisionMeaning = interpretation.acceptedConstraintMutations.length > 0
        || interpretation.acceptedBudgetMutations.length > 0
        || interpretation.result.modelReferences.length > 0
        || interpretation.result.directAnswerRequests.some((request) => ["RECOMMENDATION_REQUEST", "MODEL_AVAILABILITY", "MODEL_COMPARISON", "MODEL_SUITABILITY", "TECHNICAL_EXPLANATION"].includes(request.kind))
        || /(?:araç|araba|otomobil|suv|sedan|hatchback|crossover|pickup|yakıt|benzin|dizel|hibrit|elektrik|motor|şanzıman|çekiş|menzil|bagaj|koltuk|bütçe|fiyat|model|marka)/iu.test(turn.userMessage);
      const structuralDecisionAmbiguity = interpretation.result.ambiguities.some((ambiguity) =>
        ambiguity.code === "UNBOUND_SHORT_CONFIRMATION"
        || /\b(?:ikisi|ikisini|ikisi de|her ikisi|hepsi|hepsi de)\b/iu.test(ambiguity.sourceSpan)
        || /^\s*\d+(?:[.,]\d+)?\s*m\s*$/iu.test(ambiguity.sourceSpan));
      const casualConversationalQuestion = !currentMessageHasDecisionMeaning
        && /(?:izledin mi|gördün mü|dinledin mi|takip ettin mi|haber(?:in)? var mı)[?？]?\s*$/iu.test(turn.userMessage);
      const casualQuestionAcknowledgement = casualConversationalQuestion
        ? /maç|karşılaşma/iu.test(turn.userMessage)
          ? "Ben maç izleyemiyorum; ama belli ki konuşmaya değer bir karşılaşmaydı 🙂 Hangi maçtan söz ediyorsun?"
          : "Ben olayları canlı olarak izleyemiyorum; ama neyi kastettiğini söylersen üzerine sohbet edebiliriz. 🙂"
        : undefined;
      let ambiguityClarification = contextualPriceQuestion || detectedHumanContext || semanticIntelligenceQuestion || governedEconomicMeaningQuestion || casualPersonalUpdate || casualConversationalQuestion || (!currentMessageHasDecisionMeaning && !structuralDecisionAmbiguity) ? undefined : clarificationForAmbiguity(turn.userMessage, interpretation.result.ambiguities);
      if (ambiguityClarification && !semanticIntelligenceQuestion && act !== "ABUSE") action = { ...action, nextState: "UNDERSTANDING_NEEDS", nextAction: { type: "ANSWER_DIRECTLY" }, directAnswerObligation: { kind: "OTHER_SUPPORTED", sourceMessageId: turn.messageId, authorizedExplanationFactIds: ["direct-answer"], authorizedCandidateIds: [], placement: "BEFORE_MATERIAL_QUESTION" }, materialQuestion: null, shortlistIntent: null, explanationFactIds: [...new Set([...action.explanationFactIds, "direct-answer"])], policyTrace: { ...action.policyTrace, matchedRule: "AMBIGUOUS_INTERPRETATION_REQUIRES_CONFIRMATION", directAnswerPlacement: "BEFORE_MATERIAL_QUESTION" } };
      if (casualPersonalUpdate && act !== "ABUSE") action = { ...action, nextState: "SOCIAL", nextAction: { type: "ANSWER_DIRECTLY" }, directAnswerObligation: { kind: "OTHER_SUPPORTED", sourceMessageId: turn.messageId, authorizedExplanationFactIds: ["direct-answer"], authorizedCandidateIds: [], placement: "BEFORE_MATERIAL_QUESTION" }, materialQuestion: null, shortlistIntent: null, explanationFactIds: [...new Set([...action.explanationFactIds, "direct-answer"])], policyTrace: { ...action.policyTrace, matchedRule: "CASUAL_PERSONAL_UPDATE_ACKNOWLEDGEMENT", directAnswerPlacement: "BEFORE_MATERIAL_QUESTION" } };
      if (casualConversationalQuestion && act !== "ABUSE") action = { ...action, nextState: "SOCIAL", nextAction: { type: "ANSWER_DIRECTLY" }, directAnswerObligation: { kind: "OTHER_SUPPORTED", sourceMessageId: turn.messageId, authorizedExplanationFactIds: ["direct-answer"], authorizedCandidateIds: [], placement: "BEFORE_MATERIAL_QUESTION" }, materialQuestion: null, shortlistIntent: null, explanationFactIds: [...new Set([...action.explanationFactIds, "direct-answer"])], policyTrace: { ...action.policyTrace, matchedRule: "CASUAL_CONVERSATIONAL_QUESTION", directAnswerPlacement: "BEFORE_MATERIAL_QUESTION" } };
      ...
      const humanContextAcknowledgement = contextualSocialAcknowledgement ? socialText : undefined;
      const conversationalAcknowledgement = greetingAcknowledgement ?? humanContextAcknowledgement;
      const preferenceAcknowledgement = conversationalAcknowledgement && decisionAcknowledgement ? `${conversationalAcknowledgement}\n\n${decisionAcknowledgement}` : conversationalAcknowledgement ?? decisionAcknowledgement;
      ...
      const purchaseIntentEvidenced = memory.vehicleIntentEstablished
        || constraints.activeHardConstraints.length > 0
        || constraints.activeNonHardConstraints.length > 0
        || memory.modelReferences.some((reference) => ["PREFERENCE", "HARD_SCOPE", "COMPARISON_SCOPE"].includes(reference.decisionEffect))
        || !memory.budget.budgetUnknown
        || memory.directAnswerHistory.some((item) => item.obligation === "RECOMMENDATION_REQUEST");
      const salesPhase = salesConversationPhase({ turn: memory.turn, vehicleIntentEstablished: purchaseIntentEvidenced });
      const salesGuidance = salesConversationGuidance(salesPhase);
      const lightWeatherSmallTalk = act === "OFF_TOPIC"
        && /(?:hava.{0,30}(?:sıcak|soğuk|güzel|bunaltıcı)|(?:sıcak|soğuk|güzel|bunaltıcı).{0,30}hava)/iu.test(turn.userMessage)
        && !/(?:nasıl|kaç derece|yağacak mı|hava durumu|tahmin)/iu.test(turn.userMessage);
      if (salesPhase === "OPEN_CONVERSATION" && lightWeatherSmallTalk) action = { ...action, nextState: "SOCIAL", nextAction: { type: "ANSWER_DIRECTLY" }, directAnswerObligation: { kind: "OTHER_SUPPORTED", sourceMessageId: turn.messageId, authorizedExplanationFactIds: ["direct-answer"], authorizedCandidateIds: [], placement: "BEFORE_MATERIAL_QUESTION" }, materialQuestion: null, shortlistIntent: null, explanationFactIds: [...new Set([...action.explanationFactIds, "direct-answer"])], policyTrace: { ...action.policyTrace, matchedRule: "OPEN_CONVERSATION_BEFORE_PURCHASE_INTENT" } };
      if (salesPhase === "INTENT_NURTURE_1" || salesPhase === "INTENT_NURTURE_2") action = { ...action, nextState: "SOCIAL", nextAction: { type: "ANSWER_DIRECTLY" }, directAnswerObligation: { kind: "OTHER_SUPPORTED", sourceMessageId: turn.messageId, authorizedExplanationFactIds: ["direct-answer"], authorizedCandidateIds: [], placement: "BEFORE_MATERIAL_QUESTION" }, materialQuestion: null, shortlistIntent: null, explanationFactIds: [...new Set([...action.explanationFactIds, "direct-answer"])], policyTrace: { ...action.policyTrace, matchedRule: salesPhase } };
      if (salesPhase === "END_WITHOUT_PURCHASE_INTENT") action = { ...action, nextState: "LIMITED_OR_ENDED", nextAction: { type: "END_POLITELY" }, directAnswerObligation: { kind: "OTHER_SUPPORTED", sourceMessageId: turn.messageId, authorizedExplanationFactIds: ["direct-answer"], authorizedCandidateIds: [], placement: "BEFORE_MATERIAL_QUESTION" }, materialQuestion: null, shortlistIntent: null, explanationFactIds: [...new Set([...action.explanationFactIds, "direct-answer"])], policyTrace: { ...action.policyTrace, matchedRule: salesPhase } };
      ...
      const activeQuestionCohortCount = constraints.activeNonHardConstraints.length > 0 ? decisionCohortIds.length : technical.eligibleCandidateIds.length;
      const candidateSummary = { count: activeQuestionCohortCount, basis: budgetShortlistApplicable && presentingShortlist ? "BUDGET_NEAREST_SHORTLIST" as const : "ACTIVE_DECISION_COHORT" as const, label: budgetShortlistApplicable && presentingShortlist ? `${technical.eligibleCandidateIds.length} seçenek teknik değerlendirmede; doğrulanmış veya kontrollü tahmini fiyatı bütçene en yakın ${shortlist.candidateIds.length} seçenek hazırlandı.` : `${activeQuestionCohortCount} seçenek değerlendirmede.` };
      ...
      input.smokeObserver?.({ phase: "SEMANTIC_AUTHORITY", messageId: turn.messageId, providerActs: interpretation.providerActs, authoritativeActs: interpretation.result.acts, policyTrace: interpretation.policyTrace, automotiveProviderStatus: interpretation.automotiveSemantics?.providerStatus ?? "NOT_RUN", automotiveFallbackReason: interpretation.automotiveSemantics?.fallbackReason ?? null, automotiveConcepts: interpretation.automotiveSemantics?.concepts.map((signal) => ({ concept: signal.concept, explicitness: signal.explicitness, confirmationStatus: signal.confirmationStatus })) ?? [], semanticDecisionMutationCount: interpretation.semanticCompilation?.decisionMutations.length ?? 0, semanticPersonaMutationCount: interpretation.semanticCompilation?.personaMutations.length ?? 0, semanticWithheldCount: interpretation.semanticCompilation?.withheldSignals.length ?? 0 });
      const exposeCandidateSummary = purchaseIntentEvidenced && (candidateSummary.count < catalog.variants.length || presentingShortlist);
      return { action, offer, offerToken, offerTransition, ...(exposeCandidateSummary ? { candidateSummary } : {}), facts };
    },

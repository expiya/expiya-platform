...
describe("production V2 composition with real WP pipeline", () => {
  ...
  it("does not answer an unasked how-are-you question and repairs the conversational mistake", async () => {
    ...
    const hello = await runCarsDecisionTurnV2({ conversationId: "social-repair", messageId: "hello", idempotencyKey: "hello", expectedConversationRevision: 0, userMessage: "merhaba", requestTime: "2026-08-19T00:00:00.000Z" }, composition);
    expect(hello.message).toBe("Merhaba! Hoş geldin."); expect(hello.message).not.toMatch(/iyiyim/iu);
    const repair = await runCarsDecisionTurnV2({ conversationId: "social-repair", messageId: "repair", idempotencyKey: "repair", expectedConversationRevision: 1, userMessage: "Ben sana nasılsın diye sormadım.", requestTime: "2026-08-19T00:01:00.000Z" }, composition);
  ...
  it("acknowledges first-car excitement and binds a bare daily answer to the open usage question", async () => {
    ...
    const first = await runCarsDecisionTurnV2({ conversationId: "first-car-daily", messageId: "first", idempotencyKey: "first", expectedConversationRevision: 0, userMessage: "İyiyim, ilk arabamı almak için heyecanlıyım.", requestTime: "2026-08-19T00:00:00.000Z" }, composition);
    ...
    await runCarsDecisionTurnV2({ conversationId: "first-car-daily", messageId: "daily", idempotencyKey: "daily", expectedConversationRevision: 1, userMessage: "günlük", requestTime: "2026-08-19T00:01:00.000Z" }, composition);
  ...
  it("acknowledges human context once without turning it into a vehicle constraint", async () => {
    ...
    const first = await runCarsDecisionTurnV2({ conversationId: "human-context-once", messageId: "urgent", idempotencyKey: "urgent", expectedConversationRevision: 0, userMessage: "Acil araba almam gerekiyor.", requestTime: "2026-08-19T00:00:00.000Z" }, composition);
    ...
    const repeated = await runCarsDecisionTurnV2({ conversationId: "human-context-once", messageId: "urgentAgain", idempotencyKey: "urgentAgain", expectedConversationRevision: 1, userMessage: "Acil araba almam gerekiyor.", requestTime: "2026-08-19T00:01:00.000Z" }, composition);
  ...
  it("asks a conversation-local clarification for an ambiguous five-door expression", async () => {
    ...
    const output = await runCarsDecisionTurnV2({ conversationId: "five-door-recovery", messageId: "fiveDoor", idempotencyKey: "fiveDoor", expectedConversationRevision: 0, userMessage: "Günlük şehir içi kullanacağım, 5 kapılı bir araç olsun.", requestTime: "2026-08-19T00:00:00.000Z" }, composition);
  ...
  it("binds a bare amount to the open budget question and immediately creates the governed offer", async () => {
    ...
    let output = await runCarsDecisionTurnV2({ conversationId, messageId: "start", idempotencyKey: "start", expectedConversationRevision: revision++, userMessage: "Araba alacağım", requestTime: "2026-08-19T00:00:00.000Z" }, composition);
    for (let step = 0; step < 5 && !output.offer; step += 1) {
      ...
      output = await runCarsDecisionTurnV2({ conversationId, messageId: `answer-${step}`, idempotencyKey: `answer-${step}`, expectedConversationRevision: revision++, userMessage: answer, requestTime: `2026-08-19T00:0${step + 1}:00.000Z` }, composition);
    }
  ...
  it("supersedes a redirected material question and does not let it block the next stage or final offer", async () => {
    ...
    output = await runCarsDecisionTurnV2({ conversationId, messageId: "redirect", idempotencyKey: "redirect", expectedConversationRevision: revision++, userMessage: "Kullanım ayrıntısını sonra konuşalım, önce nasıl bir araç istediğimizi belirleyelim", requestTime: "2026-08-19T00:01:00.000Z" }, composition);
  ...
  it("keeps the real manual diesel AWD pickup candidate eligible", async () => {
    ...
    const output = await runCarsDecisionTurnV2({ conversationId: "manual-pickup", messageId: "pickup", idempotencyKey: "pickup", expectedConversationRevision: 0, userMessage: "Köyde kullanacağım. Pickup olmalı, 4x4 şart, dizel olmalı ve manuel olmalı.", requestTime: "2026-08-19T00:00:00.000Z" }, composition);
  ...
  it("reaches an offer after completing the off-road pickup discovery path", async () => {
    const traces: Readonly<Record<string, unknown>>[] = []; const store = new InMemoryV2ConversationStore(); const offerStore = new InMemoryGovernedOfferStore();
    const interpreter: StructuredInterpretationModel = { interpret: async (request) => result(request.messageId, []) };
    const composition = createCarsDecisionV2ProductionComposition({ store, offerStore, interpreter, realizer, signer: createHmacOfferSigner({ secret: "01234567890123456789012345678901", now: () => new Date("2026-08-19T00:06:00.000Z") }), smokeObserver: (trace) => traces.push(trace) });
    const messages = ["merhaba. arazi aracı bakıyorum.", "Pickup", "Dört çeker", "fark etmez", "Manuel", "4 milyon tl"] as const;
    let output;
    for (let index = 0; index < messages.length; index += 1) output = await runCarsDecisionTurnV2({ conversationId: "off-road-pickup-offer", messageId: `offroad-${index}`, idempotencyKey: `offroad-${index}`, expectedConversationRevision: index, userMessage: messages[index]!, requestTime: `2026-08-19T00:0${index}:00.000Z` }, composition);
  ...
  it("explains the current traction question instead of replaying an older model lookup", async () => {
    ...
    await runCarsDecisionTurnV2({ conversationId: "lookup-then-traction", messageId: "lookup", idempotencyKey: "lookup", expectedConversationRevision: 0, userMessage: "Clio var mı?", requestTime: "2026-08-19T00:00:00.000Z" }, composition);
    const explained = await runCarsDecisionTurnV2({ conversationId: "lookup-then-traction", messageId: "explain", idempotencyKey: "explain", expectedConversationRevision: 1, userMessage: "FWD, AWD ve RWD bunların anlamı ne?", requestTime: "2026-08-19T00:01:00.000Z" }, composition);
  ...
  it("persists an identity-free offer and reveals only its exact authorized cards after consent", async () => {
    ...
    const revealed = await runCarsDecisionTurnV2({ conversationId: "composition-offer", messageId: "consent", idempotencyKey: "consent", expectedConversationRevision: offered.revision, userMessage: "Paylaş", offerToken: offered.offer!.token, requestTime: "2026-08-19T00:06:00.000Z" }, composition);
  ...
  it("does not broaden confirmed functional preferences after every matching revealed variant is rejected", async () => {
    ...
    const interpreter: StructuredInterpretationModel = { interpret: async (request) => result(request.messageId, []) };
    const composition = createCarsDecisionV2ProductionComposition({ store, offerStore, interpreter, realizer, signer: createHmacOfferSigner({ secret: "01234567890123456789012345678901", now: () => new Date("2026-08-19T00:04:00.000Z") }) });
    const firstOffer = await runCarsDecisionTurnV2({ conversationId: "plural-rejection", messageId: "first", idempotencyKey: "first", expectedConversationRevision: 0, userMessage: "Uzun yol için hızlı elektrikli coupe istiyorum, otomatik olsun; bütçe önemli değil.", requestTime: "2026-08-19T00:00:00.000Z" }, composition);
    expect(firstOffer.offer?.token).toBeTruthy();
    const firstReveal = await runCarsDecisionTurnV2({ conversationId: "plural-rejection", messageId: "reveal-first", idempotencyKey: "reveal-first", expectedConversationRevision: 1, userMessage: "göster", offerToken: firstOffer.offer!.token, requestTime: "2026-08-19T00:01:00.000Z" }, composition);
    const rejectedIds = firstReveal.cards.map((card) => card.exactVariantId); expect(rejectedIds.length).toBeGreaterThan(0);
    const replacementOffer = await runCarsDecisionTurnV2({ conversationId: "plural-rejection", messageId: "reject", idempotencyKey: "reject", expectedConversationRevision: 2, userMessage: "Bunları beğenmedim. Bütçem 15 milyon mümkün.", requestTime: "2026-08-19T00:02:00.000Z" }, composition);
  ...
  it("keeps offer authorization and reveal deterministic across twenty replays even with realization fallback", async () => {
    ...
    for (let index = 0; index < 20; index += 1) {
      ...
      const revealed = await runCarsDecisionTurnV2({ conversationId: `determinism-${index}`, messageId: "consent", idempotencyKey: "consent", expectedConversationRevision: offered.revision, userMessage: "Göster bakalım", offerToken: offered.offer!.token, requestTime: "2026-08-19T00:05:00.000Z" }, composition);

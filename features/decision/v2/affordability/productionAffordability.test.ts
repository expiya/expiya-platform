...
describe("WP6 production affordability diagnostic", () => {
  it("evaluates all 577 pinned candidates with injected post-effective time", async () => {
    ...
    expect(first).toEqual(second); expect(first.initialCandidateIds).toHaveLength(577);
    ...
    expect(first.internalEstimateWithinCandidateIds.length + first.estimatedOverBudgetConditionalCandidateIds.length).toBe(394);

...
export function projectUsageCargoNeedFromConstraints(events: readonly ConstraintEvent[]): UsageCargoNeed {
  ...
  return Object.freeze({
    commercialScenario: urban ? "URBAN_DELIVERY" : validArchitecture === "ENCLOSED_CARGO" || validArchitecture === "OPEN_CARGO" ? "GENERAL_CARGO" : "UNSPECIFIED",
    orientation: validArchitecture === "ENCLOSED_CARGO" || validArchitecture === "OPEN_CARGO" ? "CARGO_PRIORITY" : "UNKNOWN",
    ...(validArchitecture ? { architectureRequirement: { required: validArchitecture, explicitness: architecture?.decisionEffect === "HARD_FILTER" ? "USER_EXPLICIT" as const : "INFERRED" as const } } : {}),

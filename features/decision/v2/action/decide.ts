...
export function decideConversationAction(input: DecideActionInput): ConversationActionDecision {
  ...
  else if (input.normalizedUserAct === "CORRECTION" || input.normalizedUserAct === "REJECTION" || input.systemCorrectionRequired) { if (input.recommendationReadiness === "READY_FOR_OFFER") { nextState = "AWAITING_CONSENT"; nextAction = { type: "REQUEST_REVEAL_CONSENT" }; shortlistIntent = shortlist(input); materialQuestion = null; rule = input.normalizedUserAct === "REJECTION" ? "REJECTION_RECOMPUTE_READY_SHORTLIST" : "CORRECTION_RECOMPUTE_READY_SHORTLIST"; } else { nextState = "FILTERING"; nextAction = materialQuestion ? { type: "ASK_MATERIAL_QUESTION" } : { type: "ANSWER_DIRECTLY" }; rule = "EXPLICIT_MUTATION_RECOMPUTE"; } }
  ...
  else if ((input.normalizedUserAct === "RECOMMENDATION_REQUEST" || input.memory.directAnswerHistory.some((item) => item.obligation === "RECOMMENDATION_REQUEST") || input.memory.materialQuestionHistory.some((item) => ["ANSWERED", "DECLINED"].includes(item.answerStatus))) && input.recommendationReadiness === "READY_FOR_OFFER") { nextState = "AWAITING_CONSENT"; nextAction = { type: "REQUEST_REVEAL_CONSENT" }; rule = "COMPLETED_DISCOVERY_READY_SHORTLIST"; shortlistIntent = shortlist(input); materialQuestion = null; }

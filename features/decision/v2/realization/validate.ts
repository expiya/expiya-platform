...
export function validateNaturalRealization(input: RealizationInput, result: NaturalRealizationResult): RealizationValidation {
  ...
  if (!input.materialQuestion && !input.directAnswer && input.actionDecision.nextAction.type !== "REQUEST_REVEAL_CONSENT" && /\?/u.test(message)) codes.push("QUESTION_NOT_AUTHORIZED");

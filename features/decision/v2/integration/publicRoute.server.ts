...
export async function tryRunCarsDecisionV2Public(input: PublicRouteV2Request, signal?: AbortSignal): Promise<PublicRouteV2Response | null> {
  ...
  const composition = createCarsDecisionV2ProductionComposition({ store, offerStore: store, semanticInterpreter: adapters.semanticInterpreter, interpreter: adapters.interpreter, realizer: adapters.realizer, signer });

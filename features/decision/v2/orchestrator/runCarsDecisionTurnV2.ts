...
export async function runCarsDecisionTurnV2(input: DecisionTurnV2Input, dependencies: { readonly store: V2ConversationStore; readonly stages: V2TurnStages }): Promise<DecisionTurnV2Output> {
  ...
  const revealTransition = evaluated.offerTransition?.to === "REVEALED";
  const cards = revealTransition && input.offerToken && dependencies.stages.authorizeCards && !requiresPostCommitCards ? await runTurnStage("CARDS", () => dependencies.stages.authorizeCards!({ token: input.offerToken!, conversationId: input.conversationId, catalog: catalog.snapshot, memory: finalMemory, now })) : [];

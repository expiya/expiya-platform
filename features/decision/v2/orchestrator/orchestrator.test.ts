...
const stages = (catalogStatus: "READY" | "UNAVAILABLE" = "READY"): V2TurnStages => ({
  ...
  evaluate: vi.fn(async () => ({ action, facts: [] })),
  realize: vi.fn(async () => ({ message: "Karar hazır.", source: "MODEL" as const })),
});
describe("single V2 turn orchestrator", () => { it("preserves the temporal blocker at August 16 and is ready after effective time", async () => { const unavailable = await runCarsDecisionTurnV2(turn({ requestTime: "2026-08-16T00:00:00.000Z" }), { store: new InMemoryV2ConversationStore(), stages: stages() }); expect(unavailable).toMatchObject({ recoverableStatus: "CATALOG_UNAVAILABLE", revision: 0 }); const ready = await runCarsDecisionTurnV2(turn(), { store: new InMemoryV2ConversationStore(), stages: stages() }); expect(ready).toMatchObject({ revision: 1, message: "Karar hazır.", cards: [] }); }); it("returns the same result for same message payload and rejects changed payload", async () => { const store = new InMemoryV2ConversationStore(); const pipeline = stages(); const first = await runCarsDecisionTurnV2(turn(), { store, stages: pipeline }); const replay = await runCarsDecisionTurnV2(turn(), { store, stages: pipeline }); expect(replay).toEqual(first); expect(pipeline.interpret).toHaveBeenCalledTimes(1); await expect(runCarsDecisionTurnV2(turn({ userMessage: "different" }), { store, stages: pipeline })).rejects.toMatchObject({ code: "MESSAGE_PAYLOAD_CONFLICT" }); }); });

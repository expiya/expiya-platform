...
import { V3_ROUTES, type PurchaseIntentState, type RouterResult, type V3MessageAct, type V3SemanticContextSignal } from "./types";
...
export async function interpretV31Message(input: { readonly message: string; readonly hasPurchaseIntent: boolean; readonly hasOpenQuestion: boolean; readonly signal?: AbortSignal }): Promise<V31SemanticInterpretation> {
  ...
  const fallback = (): V31SemanticInterpretation => ({ router: deterministic, purchaseIntentAssessment: fallbackAssessment, messageActs: fallbackActs, contextSignals: fallbackContextSignals(input.message), origin: "BOUNDED_FALLBACK" });
  ...
  try {
    ...
    return { router, purchaseIntentAssessment: parsed.purchaseIntentAssessment, messageActs: parsed.messageActs, contextSignals, acknowledgement, directResponse, origin: "MODEL" };

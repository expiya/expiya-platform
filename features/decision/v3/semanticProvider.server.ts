...
const schema = z.object({
  ...
  conversationReason: z.string().min(1).max(240),
  messageActs: z
    .array(
      z.enum([
        "SOCIAL",
        "AUTOMOTIVE_QUESTION",
        "VEHICLE_PURCHASE_INTENT",
        "PREFERENCE_SIGNAL",
        "DECISION_REQUEST",
        "CORRECTION",
        "CLOSING",
  ...
  preferenceSignals: z
    .array(
      z.object({
        concept: z.literal("primaryUsage"),
        normalizedValue: z.enum(V3_USAGE_PURPOSES),
        start: z.number().int().nonnegative(),
        end: z.number().int().nonnegative(),
        confidence: z.number().min(0).max(1),
  ...
  sourceSpans: z
    ...
    .max(8),
  clarificationRequirement: z.string().max(160).nullable(),
  directResponse: z.string().max(700).nullable(),
  acknowledgement: z.string().max(320).nullable(),
});
export type V31SemanticInterpretation = {
  readonly router: RouterResult;
  readonly purchaseIntentAssessment: Extract<
    PurchaseIntentState,
    "NOT_EXPRESSED" | "POSSIBLE" | "EXPLICIT"
  >;
  ...
  readonly origin: "MODEL" | "BOUNDED_FALLBACK";
};
...
const policy = (route: RouterResult["route"], hasPurchaseIntent: boolean) => ({
  ...
  catalogEvaluationRequired:
    [
      "VEHICLE_PREFERENCE_UPDATE",
      "CORRECTION_OR_RELAXATION",
      "RECOMMENDATION_OR_OFFER",
    ].includes(route) && hasPurchaseIntent,
  directAnswerRequired: [
    "SOCIAL_CONVERSATION",
    "OFF_TOPIC_REQUEST",
    "AUTOMOTIVE_INFORMATION",
    "SAFETY_BOUNDARY",
    "CLOSING_OR_TERMINATION",
  ].includes(route),
});
const safeSpans = (
  message: string,
  ...
  spans
    .filter((item) => item.start < item.end && item.end <= message.length)
    .map((item) => ({ ...item, text: message.slice(item.start, item.end) }));
const fallbackContextSignals = (
  message: string,
): readonly V3SemanticContextSignal[] => {
  const normalized = message.toLocaleLowerCase("tr");
  const signals: V3SemanticContextSignal[] = [];
  const add = (kind: V3SemanticContextSignal["kind"], pattern: RegExp) => {
  ...
  add(
    "FIRST_TIME_DRIVER",
    /(?:ehliyet(?:imi)?(?: bugün)? aldım|ilk arac[ıi]m[ıi])/u,
  );
  add(
    "PURCHASE_RESEARCH",
    /(?:ilk )?arac[ıi]m[ıi].*(?:araştır|bakıyorum|almak)/u,
  );
  add(
    "CURRENT_VEHICLE_OWNER",
...
export async function interpretV31Message(input: {
...
}): Promise<V31SemanticInterpretation> {
  const deterministic = routeConversationMessage(input.message, input);
  const fallbackAssessment = deterministic.purchaseIntentEvidence.length
    ? ("EXPLICIT" as const)
    : ("NOT_EXPRESSED" as const);
  const fallbackHasAutomotiveQuestion =
    /(?:nasıl|nedir|ne kadar|farkı|fark var|doğru mu|yeterli mi|çok mu|sorunu var mı|sıkıntısı var mı)/iu.test(
      input.message,
    ) &&
    /(?:ara[çc]|araba|otomobil|motor|şanzıman|batarya|yakıt|paket|süspansiyon|çekiş|bagaj|koltuk|servis|menzil|şarj)/iu.test(
      input.message,
    );
  const fallbackActs: readonly V3MessageAct[] =
    deterministic.route === "PURCHASE_INTENT_DISCOVERY"
      ? [
          "VEHICLE_PURCHASE_INTENT",
          ...(fallbackHasAutomotiveQuestion
            ? ["AUTOMOTIVE_QUESTION" as const]
            : []),
        ]
      : deterministic.route === "RECOMMENDATION_OR_OFFER"
        ? [
            "VEHICLE_PURCHASE_INTENT",
            "DECISION_REQUEST",
            ...(fallbackHasAutomotiveQuestion
        ...
        : deterministic.route === "AUTOMOTIVE_INFORMATION"
          ? ["AUTOMOTIVE_QUESTION"]
          : ["SOCIAL"];
  const fallbackUsage = detectExplicitUsagePurpose(input.message);
  const fallback = (): V31SemanticInterpretation => ({
  ...
  try {
        ...
        input: [
          ...
          },
          {
            ...
            content: JSON.stringify({
              ...
              hasPurchaseIntent: input.hasPurchaseIntent,
    ...
    const parsedHasPurchaseAct =
      parsed.messageActs.includes("VEHICLE_PURCHASE_INTENT") ||
      parsed.messageActs.includes("DECISION_REQUEST");
    const shortOpenQuestionAnswer =
      input.hasOpenQuestion &&
      /^(?:evet|hayır|olur|olmaz|tamam|peki|şart değil|fark etmez|istemiyorum|istiyorum)[.! ]*$/iu.test(
        input.message.trim(),
      );
    const governedRoute =
      deterministic.route === "SAFETY_BOUNDARY"
        ? deterministic.route
        : shortOpenQuestionAnswer
          ? "QUESTION_ANSWER"
          : parsed.purchaseIntentAssessment === "EXPLICIT" &&
              parsedHasPurchaseAct &&
              [
                "SOCIAL_CONVERSATION",
                "OFF_TOPIC_REQUEST",
                "AUTOMOTIVE_INFORMATION",
                "QUESTION_ANSWER",
              ].includes(parsed.route)
            ? deterministic.route === "RECOMMENDATION_OR_OFFER" ||
              parsed.messageActs.includes("DECISION_REQUEST")
              ? "RECOMMENDATION_OR_OFFER"
              : "PURCHASE_INTENT_DISCOVERY"
    ...
    const purchaseIntentEvidence = safeSpans(
      input.message,
      parsed.purchaseIntentSpans,
    );
    const router: RouterResult = {
      version: "3.8",
      route: governedRoute,
      confidence:
        governedRoute === parsed.route
          ? parsed.confidence
          : deterministic.confidence,
      ...governed,
      purchaseIntentEvidence: purchaseIntentEvidence.length
        ? purchaseIntentEvidence
        : deterministic.purchaseIntentEvidence,
      sourceSpans: safeSpans(input.message, parsed.sourceSpans),
      conversationReason:
        governedRoute === parsed.route
          ? parsed.conversationReason
          : deterministic.conversationReason,
      clarificationRequirement:
        governedRoute === parsed.route
          ? parsed.clarificationRequirement
          : deterministic.clarificationRequirement,
    };
    const directResponseRequired =
      governed.directAnswerRequired ||
      parsed.messageActs.includes("AUTOMOTIVE_QUESTION");
    const usedAdviceInNewCarFlow =
      [
        "PURCHASE_INTENT_DISCOVERY",
        "RECOMMENDATION_OR_OFFER",
        "VEHICLE_PREFERENCE_UPDATE",
      ].includes(governedRoute) &&
      /(?:ikinci\s*el|ekspertiz|hasar kaydı|bakım geçmişi|düşük kilometre)/iu.test(
        parsed.directResponse ?? "",
      );
    const directResponse =
      directResponseRequired &&
      parsed.directResponse &&
      !usedAdviceInNewCarFlow &&
      !/\b(?:candidate|ledger|route)\b|\d+\s+(?:aday|seçenek)/iu.test(
        parsed.directResponse,
      )
        ? parsed.directResponse
        : undefined;
    ...
    const preferenceSignals = parsed.preferenceSignals.flatMap((item) =>
      safeSpans(input.message, [item]).map((sourceSpan) => ({ concept: item.concept, normalizedValue: item.normalizedValue, sourceSpan, confidence: item.confidence, explicit: item.explicit })),
    );
    const acknowledgement =
      parsed.acknowledgement &&
      !parsed.acknowledgement.includes("?") &&
      !/\b(?:candidate|ledger|route)\b|\d+\s+(?:aday|seçenek)/iu.test(
        parsed.acknowledgement,
      )
        ? parsed.acknowledgement

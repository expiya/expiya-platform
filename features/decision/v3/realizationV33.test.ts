...
describe("V3.3 Turkish and daily-use realization", () => {
  it("uses a user-specific daily-use example from conversation history", async () => {
    ...
    expect(output.message).toMatch(/senin de söylediğin gibi, okul yolculuklarını/iu); expect(output.message).toMatch(/geri görüş kamerası|park sensörleri/iu);
  });
  ...
  it("reveals only brand/model titles without examples, details or experience copy", async () => {
    ...
    expect(output.recommendations).toHaveLength(1); expect(Object.keys(output.recommendations![0]!).sort()).toEqual(["id", "title"]);

...
export function analyzeSemanticNeedsFallback(input: { message: string; sourceMessageId: string; conversationRevision: number }): SemanticNeedsAnalysisV1 {
  ...
  const rural = span(message, /köyde (?:kullanacağım|yaşıyorum|yaşıyoruz)|kırsalda (?:kullanacağım|yaşıyorum|yaşıyoruz)|bağ bahçe işleri(?:yle)?/iu); const rough = span(message, /(?:yollar? )?(?:bozuk(?: ve stabilize)?|stabilize|toprak|asfaltsız|mıcırlı)(?: yollar?(?:da)?)?/iu);
  ...
  const charmingDesign = span(message, /(?:şirin|sevimli|sempatik|tatlı görünümlü|retro görünümlü)/iu, preferenceSearchText); if (purchaseLanguage && charmingDesign) { explicitFacts.push({ concept: "designCharacterPreference", normalizedValue: "CHARMING", sourceSpan: charmingDesign, confidence: 0.98, explicitness: "USER_EXPLICIT", confirmationRequired: false }); expressed.add("designCharacterPreference"); }

...
export function detectExplicitUsagePurpose(text: string): DetectedUsagePurpose | undefined {
  const sanitized = text.replace(/ticari(?: kullanım)?\s+(?:değil|istemiyorum)|yük\s+taşım(?:ıyorum|ayacağım)/giu, (match) => " ".repeat(match.length));
  const patterns: readonly [RegExp, V3UsagePurpose][] = [
    ...
    [/(?:kamp|arazi|bozuk(?:\s+ve\s+\p{L}+)?\s+yol|stabilize\s+yol|toprak\s+yol|mıcır(?:lı)?\s+yol|asfaltsız\s+yol|engebeli\s+yol|çamurlu\s+yol|köy\s+yol|köyde.{0,40}(?:kullan|sür|git|araç)|kırsal(?:da|\s+yol)|yayla|bağ evi|çiftlik yol|patika|doğa(?:ya|da)|4x4|dört çeker)|(?:bisiklet|kamp|açık hava|outdoor|spor).{0,50}(?:malzeme|ekipman)/iu, "MIXED_ROAD"],
    [/(?:aile|çocuk(?!luğum|\s*oyuncağı)|bebek|puset|kalabalık sülale|yaşlı (?:ann|bab)|annemi|babamı|eşimle|çocuklarımla)/iu, "FAMILY"],

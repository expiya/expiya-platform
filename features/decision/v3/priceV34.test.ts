...
import { evaluateV3Catalog, v34MatchesBudget, v34PriceAuthority } from "./catalogAdapter.server";
...
describe("V3.4 estimated and unavailable price governance", () => {
  it("uses estimated amounts in a hard-budget query and retains vehicles with no price", async () => {
    ...
    const filtered = await evaluateV3Catalog(state.ledger); const retained = new Set(filtered.candidateIds);
  ...
  it("never projects a price field on a revealed public card", async () => {
    ...
    expect(output.recommendations).toHaveLength(1); expect(Object.keys(output.recommendations![0]!).sort()).toEqual(["id", "title"]); expect(JSON.stringify(output.recommendations)).not.toMatch(/price|fiyat|amount/iu);
  });
});

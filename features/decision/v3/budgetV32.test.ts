...
describe("V3.2 budget timing and authority", () => {
  it("asks budget immediately after usage and body become hard filters", async () => {
    ...
    output = await turn(state, "Şehir içinde günlük kullanacağım", "2"); state = output.state; expect(output.message).toMatch(/geri görüş kamerası|park sensörleri/iu);
    output = await turn(state, "Özel bir donanım şart değil", "3"); state = output.state; expect(output.message).toMatch(/Park kolaylığı mı/iu);
    output = await turn(state, "Daha ferah ve yüksek olsun", "4");
    expect(output.message).toMatch(/düşündüğün bütçe/iu);
  ...
  it("asks for an exact ceiling after other filter questions are exhausted", async () => {
    let state = createV3ConversationState("exact-last"); let output;
  ...
  it("uses brand/model once when budget is irrelevant, then prepares three value choices", async () => {
    ...
    output = await turn(state, "Fark etmez, sen seç", "5"); expect(output.offerAwaitingConsent).toBe(true); expect(output.state.pendingOffer?.candidateIds.length).toBeLessThanOrEqual(3);
    output = await turn(output.state, "Evet, göster", "6"); expect(output.recommendations?.length).toBeGreaterThan(0); expect(output.recommendations?.length).toBeLessThanOrEqual(3);
  });
});

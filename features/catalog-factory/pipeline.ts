...
export async function runCatalogFactory(input: FactoryInput, options: FactoryRunOptions = {}): Promise<FactoryOutput> {
  ...
  const base = { schemaVersion: "catalog-factory-output/v0.1" as const, inputDigest, stages: FACTORY_STAGES, coverage, gates, domainPackCandidates, integrationManifest: { status: "PENDING_OWNER_APPROVAL" as const, consumers: ["DEPARTMENT_REGISTRY", "ROOT_DISCOVERY", "EXPİYA_NEDİR", "SECRETARY_IDENTITY_ROUTING", "UNIVERSAL_CARDS", "PERSISTENCE", "STAGE2_READINESS"] as const, registryMutation: "FORBIDDEN" as const, categoryDigests }, activationPlan: { immutable: true as const, status: "PLANNED_NOT_AUTHORIZED" as const, ownerApprovalRequired: true as const, activePointerWrite: false as const, databaseMigration: false as const, deployment: false as const }, checkpoint };

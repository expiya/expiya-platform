...
interface ProgramLedger {
  catalogAuthority: { productCount: number; categoryCount: number; headphonesCount: number };
  categories: { productCount: number }[];
  statuses: Record<string, boolean>;
}

interface MatrixCell { categoryId: string }
interface ObservationFact { key: string }
interface MaterialQuestion { evidenceFactKey: string; askOnlyWhenCandidatesDiffer: boolean }
...
interface DomainPack { retainedObservationCount: number; observations: Observation[] }
interface ApprovalPackage { decision: string; counts: { matrixCells: number } }
...
describe("electronics all-category bounded candidate", () => {
  it("preserves the activated 68-product / 24-category / 18-headphones authority", () => {
    const ledger = read<ProgramLedger>("progress-ledger.json");
  ...
  it("records Amazon API unavailability once without blocking technical or XPY readiness", () => {
    const status = read<{ status: string; recordedOnce: boolean; attemptedInThisRun: boolean }>("amazon-channel-status.json");
    const ledger = read<ProgramLedger>("progress-ledger.json");

...
export const ELECTRONICS_WAVE_3_VERSION = "ELECTRONICS-WAVE-3-EVIDENCE-TR-v0.1" as const;
export const ELECTRONICS_WAVE_3_PARENT_DIGEST = "sha256:824b159a07e7e6c586b687627bb29591a417454c6d475cbbcdba7dec915598f1" as const;
export const ELECTRONICS_WAVE_3_CATEGORY_IDS = ELECTRONICS_CATEGORY_REGISTRY.filter(row => row.wave === 3).map(row => row.categoryId) as readonly ElectronicsCategoryId[];
export type Wave3Readiness = "DECISION_EVIDENCE_READY" | "POLICY_REVIEW_REQUIRED" | "INFORMATION_ONLY" | "BLOCKED_EVIDENCE";

export interface Wave3EvidenceRelease {
  readonly schemaVersion: "electronics-wave-3-evidence/v1"; readonly releaseVersion: typeof ELECTRONICS_WAVE_3_VERSION;
  readonly parent: { readonly releaseDigest: typeof ELECTRONICS_WAVE_3_PARENT_DIGEST; readonly relationship: "IMMUTABLE_CHILD_NO_OVERWRITE" };
  ...
  readonly categoryReadiness: readonly { readonly categoryId: ElectronicsCategoryId; readonly candidateCount: number; readonly manufacturerCount: number; readonly comparableFieldCount: number; readonly readiness: Wave3Readiness; readonly reasons: readonly string[]; readonly policyStatus: "REVIEW_REQUIRED_NON_ACTIVE" }[];
  ...
  readonly carryForward: { readonly scope: "WAVES_1_AND_2_PARENT_CHAIN"; readonly parentReleaseDigest: typeof ELECTRONICS_WAVE_3_PARENT_DIGEST; readonly parentBytesUnmodified: true };
...
export function validateWave3EvidenceClosure(release: Wave3EvidenceRelease): readonly string[] {
  const issues: string[] = [];
  if (release.parent.releaseDigest !== ELECTRONICS_WAVE_3_PARENT_DIGEST || !release.carryForward.parentBytesUnmodified) issues.push("PARENT_CHAIN_INVALID");
  if (ELECTRONICS_WAVE_3_CATEGORY_IDS.length !== 6 || ELECTRONICS_WAVE_3_CATEGORY_IDS.some(id => !release.categoryReadiness.some(row => row.categoryId === id))) issues.push("WAVE_3_COVERAGE_INVALID");
  ...
  if (release.products.some(row => row.unresolvedIdentityDiscriminators.length || !ELECTRONICS_WAVE_3_CATEGORY_IDS.includes(row.categoryId) || !row.trApplicabilitySourceIds.some(id => release.sources.some(source => source.sourceId === id && source.market === "TR" && source.trApplicabilityAuthority === "EXACT")))) issues.push("EXACT_TR_IDENTITY_INVALID");
  ...
  if (release.categoryReadiness.some(row => row.candidateCount < 2 || row.manufacturerCount < 2 || row.comparableFieldCount < 4 || row.readiness !== "DECISION_EVIDENCE_READY" || row.policyStatus !== "REVIEW_REQUIRED_NON_ACTIVE")) issues.push("READINESS_OR_DIVERSITY_INVALID");
  if (release.manuals.some(row => !productIds.has(row.exactProductId) || !/^sha256:[a-f0-9]{64}$/u.test(row.sha256) || !row.locators.length)) issues.push("MANUAL_BINDING_INVALID");
  if (release.decisionProjections.some(row => row.eligibleFactIds.some(id => !factIds.has(id)) || row.status !== "DRAFT_NON_ACTIVE" || row.unknownTreatment !== "NEUTRAL_FAIL_CLOSED" || row.rankingWeights !== "NONE")) issues.push("POLICY_PROJECTION_UNSAFE");

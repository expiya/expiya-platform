...
describe("Electronics public presentation", () => {
  ...
  it("keeps root Secretary-only without a department directory", () => { const source = readFileSync(path.join(process.cwd(), "app/page.tsx"), "utf8"); expect(source).toContain("<UpperSecretary suggestions={SECRETARY_SEARCH_SUGGESTIONS} />"); expect(source).not.toContain("ROOT_DEPARTMENTS"); expect(source).not.toContain("department.href"); expect(source).not.toContain('aria-label="Expiya departmanları"'); expect(source).not.toContain("min-h-[24rem]"); });

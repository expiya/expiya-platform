...
describe("reconciled smartphone v02 activation",()=>{
 ...
 it("has a bound rollback and does not push or deploy",()=>{expect(sha(pointer.rollbackPointerFile)).toBe(pointer.rollbackPointerSha256);expect(pointer.productionDeployed).toBe(false);expect(event).toMatchObject({pushed:false,deployed:false,deploymentAuthorized:false});});
});

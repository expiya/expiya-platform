...
} from "@/features/persona/activeProjection";
import type { XpyCategorySoftRankingAvailability } from "./boundedSoftRanking";
...
const productAuthorityRequired = (requiredClarification: string, conflictingAuthority?: readonly string[]) => Object.freeze({ status: "FAILED_CLOSED" as const, reason: "PRODUCT_AUTHORITY_REQUIRED" as const, requiredClarification, ...(conflictingAuthority ? { conflictingAuthority } : {}) });
const categoryScopedPersona = (categories: readonly string[]) => Object.freeze({
  status: "CATEGORY_SCOPED" as const,
  authority: [UNIVERSAL_PERSONA_PROJECTION_AUTHORITY],
  categoryBindings: Object.freeze(Object.fromEntries(categories.map(category => [category, universalPersonaCategorySoftRanking(category)]))),
});
...
export function resolveXpyCategorySoftRanking(
  departmentId: keyof typeof XPY_DOMAIN_PACKS,
  categoryId: string,
): XpyCategorySoftRankingAvailability {
  const availability = XPY_DOMAIN_PACKS[departmentId].boundedSoftRanking;
  if (availability.status !== "CATEGORY_SCOPED") return availability;
  return availability.categoryBindings[categoryId] ?? productAuthorityRequired("No category-local Persona mapping is active.");
}

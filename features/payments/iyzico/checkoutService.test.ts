...
describe("iyzico checkout service", () => {
  it("persists initialization only after a signed provider response", async () => {
    ...
    await expect(startIyzicoCheckout({ quoteId: "quote", buyer, buyerIp: "127.0.0.1", callbackUrl: "https://example.com/callback", secretKey: "secret", client, repository: repo, orderId: "order", now: new Date("2026-08-29T10:00:00Z") })).resolves.toEqual({ orderId: "order", paymentPageUrl: "https://sandbox.example/token", expiresAt: "2026-08-29T10:30:00.000Z" });
  ...
  it("marks the order failed when provider verification fails", async () => {
    ...
    await expect(startIyzicoCheckout({ quoteId: "quote", buyer, buyerIp: "127.0.0.1", callbackUrl: "https://example.com/callback", secretKey: "secret", client, repository: repo, orderId: "order" })).rejects.toThrow();

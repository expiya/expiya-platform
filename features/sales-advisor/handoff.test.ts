...
describe("signed phase 2 handoff", () => {
  ...
  it("accepts a revealed exact variant, is idempotent and leaves phase 1 state unchanged", async () => {
    ...
    const phase3Opened = await openPhase3IntentHandoff(phase3.token, "REQUEST_TEST_DRIVE"); expect(phase3Opened.handoff).toMatchObject({ selectedExactVariantId: selected, decisionRevision: output.state.revision, catalogFingerprint: opened.handoff.catalogFingerprint, authority: { departmentId: "CARS", categoryId: "NEW_CAR", exactProductId: selected, decisionRevision: output.state.revision, intendedAction: "REQUEST_TEST_DRIVE", externalExecutionAuthorized: false, replayPolicy: "REVISION_BOUND_REUSABLE_UNTIL_EXPIRY" } }); await expect(openPhase3IntentHandoff(phase3.token, "REQUEST_QUOTE")).rejects.toThrow("INTENT_MISMATCH"); await expect(openPhase3IntentHandoff(`${phase3.token}x`)).rejects.toThrow("TAMPERED"); await expect(openPhase3IntentHandoff(phase3.token, undefined, new Date(Date.now() + 31 * 60_000))).rejects.toThrow("STALE"); expect(JSON.stringify(output.state)).toBe(before);
  }, 30_000);

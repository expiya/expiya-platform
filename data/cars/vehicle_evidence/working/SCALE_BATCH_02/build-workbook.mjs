...
const base="data/cars/vehicle_evidence/working/SCALE_BATCH_02";
const input=await FileBlob.load(`${base}/Expiya_Cars_Vehicle_Evidence_SCALE_BATCH_02_WORKING.xlsx`);
...
readme.getRange("A1").values=[["Expiya Cars Vehicle Evidence Dataset v0.3.0"]];
...
readme.getRange("B4").values=[["APPROVED MASTER v0.3.0 — VALIDATOR PASS"]];
readme.getRange("B5").values=[["Vehicle Identity + Typed Evidence + Equipment + Safety + Fact-level Provenance"]];
readme.getRange("B45").values=[["0.3.0"]];
readme.getRange("B46").values=[["0.3.0"]];
readme.getRange("B47").values=[["SCALE_BATCH_02"]];
...
const output=await SpreadsheetFile.exportXlsx(workbook); await output.save(`${base}/Expiya_Cars_Vehicle_Evidence_SCALE_BATCH_02_WORKING.xlsx`);

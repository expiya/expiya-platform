      ...
      <div className="relative mx-auto flex min-h-[92svh] max-w-7xl flex-col px-5 pb-10 pt-6 sm:px-8 lg:px-12">
        ...
        <div className="my-auto max-w-3xl py-16 sm:py-24">
          <div className="flex items-center gap-3 text-xs font-semibold uppercase tracking-[.28em] text-emerald-300">
            ...
            Karar yolunu birlikte aydınlatalım
          </div>
          <h1 className="mt-7 text-5xl font-semibold leading-[.94] tracking-[-.055em] sm:text-7xl lg:text-[6.5rem]">
            Seçenek değil,<br /><span className="text-white/55">net bir yön.</span>
          </h1>
          <p className="mt-7 max-w-2xl text-base leading-7 text-white/70 sm:text-lg sm:leading-8">
            İhtiyaçlarını bir arkadaşına anlatır gibi konuş. Expiya Cars, yüzlerce varyantı kullanımına göre sadeleştirir; kararını gerekçeleriyle birlikte görünür kılar.
          </p>
        ...
        <div className="grid gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 backdrop-blur-md sm:grid-cols-3">
          {[["01", "Seni dinler", "Kullanımını ve önceliklerini doğal bir sohbetle anlar."], ["02", "Seçenekleri sınar", "Teknik gerçekleri, fiyat durumunu ve ihtiyaç uyumunu ayırır."], ["03", "Kararı netleştirir", "Tek araca kadar inebilen, gerekçeli bir karar yolu sunar."]].map(([number, title, copy]) => <article key={number} className="bg-black/35 p-5 sm:p-6">
    ...
    <section className="border-b border-white/10 bg-[#0c1113] px-5 py-14 sm:px-8 sm:py-20">
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.8fr_1.2fr] lg:items-center">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {[["eX", "Experience", "Karar sürecini bir forma değil, anlaşılır bir deneyime dönüştürür."], ["Pi", "Perspective", "İhtiyaçlarını farklı açılardan düşündüren sorularla netleştirir."], ["Ya", "You", "Sonuç, katalogdaki en popüler araca değil sana göre şekillenir."]].map(([mark, title, copy]) => <article key={mark} className="rounded-3xl border border-white/10 bg-white/[.035] p-5"><p className="text-2xl font-semibold text-emerald-300">{mark}</p><h3 className="mt-4 font-semibold">{title}</h3><p className="mt-2 text-sm leading-6 text-white/55">{copy}</p></article>)}
        </div>
      </div>
    </section>

    <div ref={conversationRef} id="sohbet" tabIndex={-1} className="scroll-mt-0 outline-none">
      {started ? children : <section className="px-5 py-16 text-center sm:py-24"><p className="text-xs font-semibold uppercase tracking-[.25em] text-emerald-300">XPY · Karar görüşmesi</p><h2 className="mx-auto mt-4 max-w-2xl text-3xl font-semibold tracking-tight sm:text-5xl">Hazır olduğunda yolu birlikte daraltalım.</h2><p className="mx-auto mt-5 max-w-xl text-white/55">Sohbet başlayınca bütçe odaklı veya yalnızca ihtiyaç odaklı ilerlemeyi sen seçersin.</p><button type="button" onClick={openConversation} className="mt-8 min-h-13 rounded-full border border-white/20 bg-white px-7 font-semibold text-stone-950 transition hover:bg-emerald-300">Sohbete başla</button></section>}
    </div>

    <footer className="border-t border-white/10 bg-black px-5 py-10 text-white/55 sm:px-8">
      <div className="mx-auto flex max-w-7xl flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-lg font-semibold text-white">EXPIYA <span className="font-light text-emerald-400">CARS</span></p><p className="mt-3 max-w-xl text-sm leading-6">Expiya Cars, SKYBIT Yazılım ve Bilgi Teknolojileri Danışmanlığı Ltd. Şti. tarafından geliştirilen yapay zekâ destekli araç karar platformudur.</p></div>

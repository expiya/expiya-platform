...
export function V2AuthorizedCarCard({ card, equipmentAction, onEquipmentExplanation, equipmentExplanationPending = false }: { readonly card: DecisionSafePublicCard; readonly equipmentAction?: EquipmentExplanationCardAction; readonly onEquipmentExplanation?: (actionId: string) => void; readonly equipmentExplanationPending?: boolean }) {
  const details = [card.modelYear, card.fuelLabel, card.transmissionLabel, card.bodyTypeLabel].filter(Boolean).join(" · ");
  return <article className="group overflow-hidden rounded-2xl border border-neutral-200 bg-white transition hover:-translate-y-1 hover:shadow-lg dark:border-neutral-700 dark:bg-neutral-900"><Link href={`/decision/v2-${encodeURIComponent(card.exactVariantId)}`} aria-label={`${card.title} ayrıntısını aç`} className="block focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-black dark:focus-visible:outline-white">

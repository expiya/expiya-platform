...
export function quickChoices(message: Message): { multiple: boolean; choices: readonly QuickChoice[] } | undefined {
  ...
  if (key === "referenceVehicleTraits:VOLKSWAGEN_PASSAT" || key === "referenceVehiclePriorities") return { multiple: true, choices: [
  ...
  if (key === "referenceVehicleTraits:DODGE_VIPER") return { multiple: true, choices: [
    { value: "Dikkat çekici spor otomobil tasarımı", label: "Dikkat çekici tasarım", description: "Alçak, geniş ve karakterli spor otomobil görünümünü öne alır." },
    { value: "Güçlü hızlanma ve performans", label: "Güçlü performans", description: "Doğrulanmış güç ve hızlanma verileri bulunan seçenekleri öne alır." },
    { value: "Sürücü odaklı sürüş karakteri", label: "Sürüş karakteri", description: "Sürüş keyfi ve sürücü odaklı karakteri daha güçlü seçenekleri öne alır." },
    { value: "Alçak spor otomobil yapısı", label: "Alçak spor otomobil", description: "SUV veya yüksek gövde yerine alçak spor otomobil yapısını öne alır." },
    { value: "Bunların dengesi", label: "Dengeli biçimde değerlendir", description: "Tek bir özelliği zorunlu tutmadan bu karakterlerin dengesine bakar.", exclusive: true },
  ] };
  ...
  if (key === "vehicleReferenceMeaning") return { multiple: false, choices: [
    { value: "Birebir marka ve model önemli", label: "Birebir araç", description: "Referanstaki gerçek marka-modelin güncel katalogda bulunup bulunmadığı kontrol edilir." },
    { value: "Görünüşü ve tasarım karakteri önemli", label: "Görünüş ve karakter", description: "Birebir model yerine benzer görsel karakter taşıyan güncel araçlar değerlendirilir." },
    { value: "Performansı ve sürüş hissi önemli", label: "Performans ve sürüş", description: "Doğrulanmış performans verileri ve sürüş karakteri öne alınır." },
    { value: "Hepsinin dengesi önemli", label: "Genel hissi", description: "Tek bir özelliği zorunlu tutmadan referansın genel karakteri üzerinden ilerlenir." },
  ] };
  if (key === "unavailableReferenceChoice") return { multiple: false, choices: [
    { value: "Bu araç hakkında bilgi ver", label: "Araç hakkında bilgi", description: "Referans aracın kimliğini ve genel otomobil tarihindeki yerini konuşur; satın alma tercihi oluşturmaz." },
    { value: "Görünüşü ve karakteri benzer güncel sıfır araçları bulalım", label: "Güncel benzerini bul", description: "Birebir model yerine aktif katalogdaki benzer karakterli sıfır araçlarla seçim görüşmesine devam eder." },
  ] };
...
export function buttonPrompt(message: Message, hasChoices: boolean): string {
...
function ConversationMessage({ message, selected, interactive, revealing, onRevealComplete, termsChecked, onTermsChecked, onAcceptTerms, onDeclineTerms, onToggle, onSend, onPhase2 }: { readonly message: Message; readonly selected: readonly string[]; readonly interactive: boolean; readonly revealing: boolean; readonly onRevealComplete: () => void; readonly termsChecked: boolean; readonly onTermsChecked: (checked: boolean) => void; readonly onAcceptTerms: () => void; readonly onDeclineTerms: () => void; readonly onToggle: (choice: QuickChoice, multiple: boolean) => void; readonly onSend: () => void; readonly onPhase2: (exactVariantId: string) => void }) {
  ...
  return <div className="max-w-[92%] space-y-1.5 sm:max-w-[82%]">
    ...
    {interactive && message.trace?.offerAwaitingConsent && <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-stone-800">
      <p className="font-semibold">Aracı göstermeden önce</p>
      <p className="mt-2 leading-6 text-stone-600">Öneri; beyan ettiğin tercihler ile tarihli katalog kaynaklarının yapay zekâ destekli ve kural tabanlı değerlendirilmesidir. Satış teklifi, garanti veya ekspertiz değildir.</p>
      <label className="mt-4 flex cursor-pointer items-start gap-3"><input type="checkbox" checked={termsChecked} onChange={(event) => onTermsChecked(event.target.checked)} className="mt-1 h-4 w-4 accent-emerald-500"/><span><Link href="/cars/arac-oneri-kosullari?returnTo=%2Fcars" className="font-semibold underline underline-offset-4">Araç Önerisi ve Katalog Kullanım Koşulları’nı</Link> ({RECOMMENDATION_TERMS_VERSION}) okudum ve kabul ediyorum.</span></label>
...
export function CarsConversationV3({ initialQuery = "", secretaryHandoff = false, minimumBudgetTry }: { readonly initialQuery?: string; readonly secretaryHandoff?: boolean; readonly minimumBudgetTry: number }) {
  ...
  const [secretaryInitialQuery, setSecretaryInitialQuery] = useState("");
  const [secretaryHandoffMissing, setSecretaryHandoffMissing] = useState(false);
  ...
  useEffect(() => { try { if (secretaryHandoff) { const pendingMessage = readSecretaryPendingMessage(sessionStorage) ?? ""; setSecretaryInitialQuery(pendingMessage); setSecretaryHandoffMissing(!pendingMessage); return; } const stored = JSON.parse(sessionStorage.getItem(storageKey) ?? "null") as { state?: V3ConversationState; stateToken?: string; messages?: Message[] } | null; if (stored?.state?.version === "3.8") { setConversationId(stored.state.conversationId); setState(stored.state); setStateToken(stored.stateToken); setMessages(stored.messages ?? []); } } finally { setRestored(true); } }, [secretaryHandoff]);
  ...
  async function openPhase2(exactVariantId: string) {
    ...
    setLoading(true);
    try { const response = await fetch("/api/cars/sales-advisor/handoff", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ conversationId: state.conversationId, stateToken, offerId: state.recommendationTermsAcceptance.offerId, selectedExactVariantId: exactVariantId }) }); const payload = await response.json() as { token?: string; error?: string }; if (!response.ok || !payload.token) throw new Error(payload.error ?? "Aşama 2 geçişi doğrulanamadı"); persistConversationBeforeNavigation(); router.push(`/cars/variant/${encodeURIComponent(exactVariantId)}?handoff=${encodeURIComponent(payload.token)}`); } catch (error) { setArchiveError(error instanceof Error ? error.message : "Aşama 2 geçişi doğrulanamadı"); setLoading(false); }
  }
  async function send(content: string, recommendationTermsAcceptance?: RecommendationTermsAcceptance) {
    ...
    try {
      ...
      if (secretaryHandoff) { clearSecretaryPendingMessage(sessionStorage); window.history.replaceState(null, "", "/cars?resume=conversation"); }
  ...
  useEffect(() => { const query = secretaryInitialQuery || initialQuery; if (!query.trim() || !restored || messages.length > 0) return; const timer = window.setTimeout(() => void send(query), 0); return () => window.clearTimeout(timer); }, [initialQuery, restored, secretaryInitialQuery]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  if (secretaryHandoffMissing) return <main className="flex min-h-screen items-center justify-center bg-[#f7f8f5] px-5 text-stone-950"><section className="max-w-lg rounded-3xl border border-stone-200 bg-white p-8 text-center shadow-sm"><h1 className="text-2xl font-semibold">Aktarım tamamlanamadı</h1><p className="mt-3 leading-7 text-stone-600">Sekreter görüşmesi bu sekmede bulunamadı. Yeniden deneyebilir veya otomobil bölümüne doğrudan girebilirsiniz.</p><div className="mt-6 flex flex-wrap justify-center gap-3"><Link href="/" className="rounded-full bg-stone-950 px-5 py-3 font-semibold text-white">Sekretere dön</Link><Link href="/cars" className="rounded-full border border-stone-300 px-5 py-3 font-semibold">Doğrudan gir</Link></div></section></main>;

...
export function UpperSecretary() {
  ...
  useEffect(() => {
    ...
    if (suggestionCharacterCount < suggestion.length) {
      const typeTimer = window.setTimeout(
        () => setSuggestionCharacterCount((current) => Math.min(current + 1, suggestion.length)),
        reducedMotion ? 0 : 28,
      );
    ...
    const nextTimer = window.setTimeout(() => {
      setSuggestionIndex((current) => (current + 1) % SECRETARY_SEARCH_SUGGESTIONS.length);
      setSuggestionCharacterCount(0);
    }, reducedMotion ? 4_000 : 1_900);
  ...
  return (
    <section aria-labelledby="secretary-title" className="mx-auto w-full max-w-3xl text-center">
      <p className="mb-4 text-xs font-bold uppercase tracking-[.22em] text-neutral-500">Çok seçenek. Tek, gerekçeli karar.</p>
      <h1 id="secretary-title" className="text-4xl font-semibold tracking-[-.055em] sm:text-6xl">Ne satın almak istiyorsunuz?</h1>
      <p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-neutral-500">İhtiyacınızı anlatın. Expiya seçenekleri güvenilir verilerle karşılaştırır, gereksiz alternatifleri eler ve size en uygun seçimi nedenleriyle birlikte ortaya çıkarır.</p>
      ...
      <form onSubmit={submit} className="relative mt-4 flex items-end gap-2 overflow-hidden rounded-[1.75rem] border border-neutral-200 bg-white p-2.5 shadow-[0_12px_45px_rgba(0,0,0,.09)] transition focus-within:border-neutral-400 focus-within:shadow-[0_16px_55px_rgba(0,0,0,.12)]">
        ...
        {!draft && <span aria-hidden="true" className="pointer-events-none absolute inset-y-0 left-6 right-16 flex items-center truncate text-left text-base text-neutral-400">{SECRETARY_SEARCH_SUGGESTIONS[suggestionIndex].slice(0, suggestionCharacterCount)}<span className="secretary-type-cursor ml-px inline-block h-[1.05em] w-px bg-neutral-400 align-[-.12em]" /></span>}
      ...
      <p className="mt-3 text-[11px] text-neutral-400">Sponsorlu sıralamalara göre değil, sizin ihtiyaç ve önceliklerinize göre.</p>
    </section>
  );
}

...
interface SecretaryResponse { readonly kind: SecretaryResultKind; readonly message: string; readonly destination?: string; readonly link?: string }
...
export function UpperSecretary() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setLoading(true); setReply("İsteğinizi anlamaya çalışıyorum…"); setReplyLink(undefined);
    try {
      ...
      setReply(payload.message); setReplyLink(payload.link); setDraft("");
  ...
  function startNewSession() { setFrozen(false); setClearViolations(0); setDraft(""); setPendingRoute(undefined); setReply(greetingForHour(new Date().getHours())); }

...
import { stableJson, verifySha256, validateGlobalEvidenceCandidate, type GlobalEvidenceAssertion } from "../features/appliances/globalEvidence";
...
type ProductIdRow = { productId: string };
type ManualBinding = ProductIdRow & { path: string; sha256: string; byteLength: number };
type ParsedCandidate = {
  members: { categoryId: string }[];
  assertions: GlobalEvidenceAssertion[];
  dailyLifeInterpretations: unknown[];
  conflicts: { productId: string; targetKey: string; disposition: string }[];
  l9AdvisorKnowledge: { advisorReadOnly: boolean; decisionAuthority: string; candidateEffect: string }[];
  boundaries: Record<string, string | boolean>;
  sources: Parameters<typeof validateGlobalEvidenceCandidate>[0]["sources"];
  manualCandidates: Parameters<typeof validateGlobalEvidenceCandidate>[0]["manuals"];
};
type ParsedManifest = { manualByteBindings: ManualBinding[]; activePointerHashesBefore: Record<string, string>; activePointerHashesAfter: Record<string, string>; candidateSha256: string; coverageSha256: string; dryRunSha256: string; researchLedgerSha256: string; sourceRegistrySha256: string; unresolvedLedgerSha256: string; completionReportSha256: string; manualExclusionsSha256: string; admittedManualsSha256: string; releaseDigest: string };

function parseProductIdRows(raw: string): ProductIdRow[] {
  const value: unknown = JSON.parse(raw);
  if (!Array.isArray(value) || !value.every((row): row is ProductIdRow => typeof row === "object" && row !== null && "productId" in row && typeof row.productId === "string")) {
    throw new Error("INVALID_ADMITTED_MANUAL_ROWS");
  }
  return value;
}
...
async function main(){
  ...
  const candidate: ParsedCandidate=JSON.parse(candidateRaw),coverage=JSON.parse(coverageRaw),dryRun=JSON.parse(dryRunRaw),manifest: ParsedManifest=JSON.parse(manifestRaw),ledger=JSON.parse(ledgerRaw),unresolved=JSON.parse(unresolvedRaw),manualFailures:string[]=[];

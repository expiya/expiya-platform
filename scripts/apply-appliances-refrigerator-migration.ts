...
async function main() {
  ...
  try {
    ...
    console.log(JSON.stringify({status:"PREFLIGHT",migration:"0011_appliances_refrigerator_product_type.sql",sha256:createHash("sha256").update(sql).digest("hex"),conversationCount:before.rows[0].count}));
    ...
    try {
      ...
      if(check.rowCount!==1||!/REFRIGERATOR/.test(check.rows[0].definition)||after.rows[0].count!==before.rows[0].count)throw new Error("MIGRATION_VERIFICATION_FAILED");
...
main().catch(()=>{console.error("APPLIANCES_REFRIGERATOR_MIGRATION_FAILED_NO_SECRET_DETAILS");process.exitCode=1;});

...
const OLD_MAP = path.join(ROOT, "data/runtime/vehicle-candidate-identity-maps/v0.1.0/mapping.json");
const MAP_DIR = path.join(ROOT, "data/runtime/vehicle-candidate-identity-maps/v0.2.0");
const ARTIFACT_DIR = path.join(ROOT, "data/runtime/vehicle-evidence/v0.2.0");
...
async function main() {
  ...
  if (em.dataset_version !== "0.4.0" || em.schema_version !== "0.1" || em.master_sha256 !== DATASET_HASH || em.validator_status !== "PASS") throw new Error("VEHICLE_EVIDENCE_RELEASE_DRIFT");
  ...
  if (JSON.stringify(oldMap.records) !== JSON.stringify(records.slice(0, 2))) throw new Error("RUNTIME_ID_REBINDING");
  const counts = { considered: 13, verifiedOneToOne: 5, unmapped: 7, ambiguous: 0, ineligible: 1 };
  const mapping: any = { mappingVersion: "0.2.0", catalogReleaseVersion: "0.2.0", catalogPayloadHash: CATALOG_HASH, catalogSchemaVersion: "0.1", catalogSourceRevision: cm.source_revision, vehicleEvidenceDatasetVersion: "0.4.0", vehicleEvidenceReleaseHash: DATASET_HASH, vehicleEvidenceSchemaVersion: "0.1", dictionaryRevision: "vehicle-evidence-0.4.0:data_dictionary.csv", dictionaryHash: sha256(dictionaryBytes), generatedAt: GENERATED_AT, validatorVersion: "vehicle-candidate-identity-map:v3", validatorStatus: "PASS", activationEligible: true, previousMappingRelease: "0.1.0", reviewReference: "catalog-v0.2.0-mapping-expansion-2026-08-15", counts, records };
  const mappingBytes = json(mapping), mappingHash = sha256(mappingBytes);
  const mappingManifest = { mappingVersion: "0.2.0", mappingHash, mappingPath: "data/runtime/vehicle-candidate-identity-maps/v0.2.0/mapping.json", catalogReleaseVersion: "0.2.0", catalogPayloadHash: CATALOG_HASH, catalogSchemaVersion: "0.1", datasetVersion: "0.4.0", datasetReleaseHash: DATASET_HASH, datasetSchemaVersion: "0.1", dictionaryRevision: mapping.dictionaryRevision, dictionaryHash: mapping.dictionaryHash, activeMappingCount: 5, statusCounts: counts, validator: { version: mapping.validatorVersion, status: "PASS" }, previousMappingRelease: "0.1.0", approvalReviewReference: mapping.reviewReference };
  ...
  const candidates = records.map((r) => { const matches = factsFor(r.configurationId, "seats"); const f = matches.find((x: any) => (assertionsByFact.get(x.fact_id) ?? []).some((a) => a.verification_status === "VERIFIED" && a.applicability_status === "EXACT")); const assertions = f ? (assertionsByFact.get(f.fact_id) ?? []).filter((a) => a.verification_status === "VERIFIED" && a.applicability_status === "EXACT") : []; return { ...r, facts: { seats: f ? { status: "AVAILABLE", value: Number(f.value), factId: f.fact_id, assertionIds: assertions.map((a) => a.assertion_id), sourceIds: [...new Set(assertions.map((a) => a.source_id))], scope: f.subject_type, evidenceState: "VERIFIED", applicability: "EXACT", limitations: [] } : { status: "MISSING", assertionIds: [], sourceIds: [], limitations: ["SEATS_EVIDENCE_UNKNOWN_IN_VEHICLE_EVIDENCE_V0.4.0"] } } }; });
  const artifact: any = { artifactVersion: "0.2.0", datasetVersion: "0.4.0", datasetReleaseHash: DATASET_HASH, schemaVersion: "0.1", mappingVersion: "0.2.0", mappingHash, dictionaryRevision: mapping.dictionaryRevision, dictionaryHash: mapping.dictionaryHash, productionCatalogRevision: cm.source_revision, generatedAt: GENERATED_AT, generatorVersion: "vehicle-evidence-runtime-artifact:v3", validationStatus: "PASS", evidenceValidator: { version: "vehicle-evidence:v0.4.0", status: "PASS" }, mappingValidator: { version: mapping.validatorVersion, status: "PASS" }, policy: { id: "cars.requirement-to-evidence", version: "0.1.0", migratedCategories: ["seats"] }, candidates, catalogReleaseVersion: "0.2.0", catalogPayloadHash: CATALOG_HASH, catalogSchemaVersion: "0.1" };
  const artifactBytes = json(artifact), artifactHash = sha256(artifactBytes);
  const artifactManifest = { artifactVersion: "0.2.0", artifactPath: "data/runtime/vehicle-evidence/v0.2.0/artifact.json", artifactSha256: artifactHash, mappingVersion: "0.2.0", mappingHash, mappingPath: mappingManifest.mappingPath, catalogReleaseVersion: "0.2.0", catalogPayloadHash: CATALOG_HASH, datasetVersion: "0.4.0", datasetReleaseHash: DATASET_HASH, dictionaryRevision: mapping.dictionaryRevision, dictionaryHash: mapping.dictionaryHash, migratedCategories: ["seats"], activeMappedCandidateCount: 5, seatsEvaluableCandidateCount: candidates.filter((c) => c.facts.seats.status === "AVAILABLE").length, validationStatus: "PASS", previousArtifactRelease: "0.1.1" };
  ...
  const report: any = { reportVersion: "0.2.0", generatedAt: GENERATED_AT, result: "CATALOG_V020_VEHICLE_EVIDENCE_V040_RECONCILIATION_VERIFIED", inputs: { catalogReleaseVersion: "0.2.0", catalogPayloadHash: CATALOG_HASH, datasetVersion: "0.4.0", datasetReleaseHash: DATASET_HASH }, metrics: { ...counts, seatsEvaluable: candidates.filter((c) => c.facts.seats.status === "AVAILABLE").length, seatsUnknown: candidates.filter((c) => c.facts.seats.status === "MISSING").length }, reconciliation, activeMappings: records, categoryCoverage, auditNotes: [{ code: "BMW_FUEL_CONSUMPTION_DISAGREEMENT", catalogVariantId: "06d935f4-6d33-4bc7-9e89-375b8db885df", catalogValueLPer100Km: 7.6, vehicleEvidenceValueLPer100Km: 7.3, disposition: "PRESERVED_NOT_MIGRATED_NON_BLOCKING_FOR_IDENTITY" }], mappingRelease: { version: "0.2.0", hash: mappingHash }, artifactRelease: { version: "0.2.0", hash: artifactHash } };
  const md = [`# Catalog v0.2.0 ↔ Vehicle Evidence v0.4.0 Reconciliation`, "", "Result: **VERIFIED**", "", "| Catalog variant | Vehicle | Evidence configuration | Result | Reason | Runtime ID |", "|---|---|---|---|---|---|", ...reconciliation.map((r: any) => `| ${r.catalogVariantId} | ${r.catalogVehicle} | ${r.candidateConfigurationId ?? "—"} | ${r.result} | ${r.reason} | ${r.runtimeVehicleCandidateId ?? "—"} |`), "", "BMW audit note: catalog 7.6 L/100 km; Vehicle Evidence 7.3 L/100 km. Preserved without reconciliation; fuel consumption is not migrated.", ""].join("\n");
  ...
  console.log(JSON.stringify({ mappingVersion: "0.2.0", mappingHash, artifactVersion: "0.2.0", artifactHash, counts, seatsEvaluable: artifactManifest.seatsEvaluableCandidateCount, categoryCoverage }));
}
